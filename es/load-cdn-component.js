...
(function () {
  ...
  function resolveAndComposeImportMap(json, baseUrl, parentMap) {
    ...
    if (json.scopes) for (var s in json.scopes) {
  ...
  systemJSPrototype.prepareImport = function () {
    if (!importMapPromise) {
      ...
      if (hasDocument) Array.prototype.forEach.call(document.querySelectorAll('script[type="systemjs-importmap"]'), function (script) {
  ...
  (function (global) {
    ...
    systemJSPrototype.instantiate = function (url, parent) {
      ...
      switch (ext) {
        ...
        case '.wasm':
          return getSourceRes().then(function (res) {
            return maybeJSFallback(res) || (WebAssembly.compileStreaming ? WebAssembly.compileStreaming(res) : res.arrayBuffer().then(WebAssembly.compile));
      ...
      function maybeJSFallback(res) {
        var contentType = res.headers.get('content-type'); // if the resource is sent as application/javascript, support eval-based execution
...
function getCacheKey(path, exportName) {
  if (exportName) {
    return "".concat(path).concat(exportName);
  } else {
    return path;
  }
}
...
function loadComponent(path, exportName) {
  ...
  function (_React$Component) {
    _inherits(LoadWrapComponent, _React$Component);

    function LoadWrapComponent() {
      var _this;

      _classCallCheck(this, LoadWrapComponent);

      _this = _possibleConstructorReturn(this, _getPrototypeOf(LoadWrapComponent).call(this));
    ...
    _createClass(LoadWrapComponent, [{
      key: "componentDidCatch",
      value: function componentDidCatch(error, info) {
        console.error(error, info);
        console.debug("loaderror -> path:".concat(path, ", exportName:").concat(exportName));
        this.setState({
          isError: true
        });
      }
      ...
      value: function componentDidMount() {
        ...
        loadScript(path, exportName).then(function (entry) {
          if (entry) {
      ...
      value: function render() {
        ...
        return C ? React.createElement(C, Object.assign({}, props, {
          ref: forwardRef
        })) : null;
      }
    }]);
  ...
  return React.forwardRef(function (props, ref) {
    return React.createElement(LoadWrapComponent, Object.assign({}, props, {
      forwardRef: ref
    }));
  });
}
